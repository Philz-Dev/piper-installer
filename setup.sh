...
# 4. Global Command Setup (The Socket & Path Fix) 
cat <<'EOF' > ./piper_wrapper 
#!/bin/bash 
USE_TTY="-it" 
FINAL_BIN="docker" 

# 🛡️ WINDOWS PATH TRANSLATION BYPASS
if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "cygwin" ]]; then 
    [ -t 0 ] && FINAL_BIN="winpty docker" || { FINAL_BIN="docker"; USE_TTY="-i"; }
    # Use double slashes to force Git Bash to ignore path translation
...
# We explicitly call it with the install path to ensure it uses the new fixed wrapper
